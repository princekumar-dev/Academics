...
class EvolutionAPIService {
  ...
  async sendTextMessage(phoneNumber, message) {
    ...
    try {
      ...
      throw new Error(`WhatsApp message failed: ${error.response?.data?.message || error.message}`);
    }
  }
  ...
  async sendMediaMessage(phoneNumber, mediaUrl, caption = '', mediaType = 'image') {
    ...
    try {
      ...
      throw new Error(`WhatsApp media message failed: ${error.response?.data?.message || error.message}`);
    }
  }
...
_This is an automated message_`;
      ...
      // Send the PDF as a WhatsApp document attachment with the full message as caption
      if (pdfUrl) {
        const message = `🎓 *MSEC Academics - Marksheet Available*\n\nDear Parent/Guardian,\n\nThe marksheet for your ward has been published:\n\n👤 *Student Name:* ${studentName}\n🆔 *Register Number:* ${registerNumber}\n📅 *Period:* ${examMonth} ${examYear}\n📝 *Examination:* ${examName}\n📊 *Result:* ${overallResult}\n\nYour ward's marksheet is now available. You can download it from the link below:\n\n📥 *Download PDF*: ${pdfUrl}\n\nFor any queries, please contact the academic department.\n\n*Meenakshi Sundararajan Engineering College*\n_This is an automated message_`;
        try {
          ...
          console.log('✅ Marksheet PDF sent successfully with message caption');
        } catch (pdfError) {
          console.error('❌ Failed to send marksheet PDF:', pdfError.message);
          throw new Error(`Failed to send marksheet PDF: ${pdfError.message}`);
        }
      }

      // Only send the PDF document with the message as caption (no separate text message)
      return {
        success: true,
        messageId: parentPhoneNumber,
