...
export async function checkUpcomingDispatches() {
  try {
    ...
    const upcomingMarksheets = await Marksheet.find({
      'dispatchRequest.status': 'approved',
    ...
    for (const marksheet of upcomingMarksheets) {
      ...
      if (marksheet.createdBy?.email) {
        await sendUserNotification(
          marksheet.createdBy.email,
          '📅 Upcoming Dispatch',
  ...
  try {
    ...
    for (const marksheet of dueMarksheets) {
      try {
        ...
        const dispatchResponse = await fetch(`${process.env.API_BASE_URL || 'http://localhost:3000'}/api/whatsapp-dispatch`, {
          method: 'POST',
