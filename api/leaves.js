            ...
            const toAbsolute = (url) => {
              ...
              try {
                ...
                try {
                  ...
                  if (abs.hostname.includes('localhost') || abs.hostname.startsWith('127.')) return ''
                  return abs.toString()
                } catch {
                  return ''
                }
...
Please find the leave approval letter attached.

📎 Download PDF: ${leavePdfUrl || 'Link unavailable'}
            ...
            try {
              ...
              if (leaveImageUrl) {
                await evolutionApi.sendMediaMessage(
                  parentPhone,
                  leaveImageUrl,
                  `Leave Approval Letter - ${request.studentDetails.name}`,
                  'image'
                )
              }
              
              console.log('✅ Leave approval sent successfully via Evolution API')
            } catch (err) {
