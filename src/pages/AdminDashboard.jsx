...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/users?action=list&userId=${auth.id}`, {
        headers: {
  ...
  const handleDeleteUser = async (userId, userEmail) => {
    try {
      const auth = JSON.parse(localStorage.getItem('auth') || '{}');
      const response = await fetch(`/api/users?action=delete&id=${userId}&userId=${auth.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
            ...
            <div className="bg-gradient-to-r from-blue-50 to-blue-100 px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-200">
              <div className="flex items-center justify-between gap-2 sm:gap-4">
                ...
                <button
                  onClick={fetchUsers}
                  disabled={loading}
                  className="flex items-center justify-center gap-2 p-2 sm:px-4 sm:py-2 bg-blue-600 text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-4 sm:p-6 md:p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              <button
                ...
                className="flex items-center justify-center gap-2 sm:gap-3 px-4 py-3 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:shadow-lg transition-all transform hover:scale-105 text-sm sm:text-base font-semibold"
              >
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 sm:gap-3 px-4 py-3 sm:py-4 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:shadow-lg transition-all transform hover:scale-105 text-sm sm:text-base font-semibold"
              >
              ...
              <button
                onClick={() => navigate('/')}
                className="flex items-center justify-center gap-2 sm:gap-3 px-4 py-3 sm:py-4 bg-gradient-to-r from-gray-600 to-gray-700 text-white rounded-lg hover:shadow-lg transition-all transform hover:scale-105 text-sm sm:text-base font-semibold"
              >
