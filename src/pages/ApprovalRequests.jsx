  ...
  const fetchPendingRequests = async () => {
    ...
    try {
      ...
      if (userData.department === 'HNS') {
        ...
        var response = await fetch(`/api/marksheets?year=I&status=dispatch_requested`)
      } else {
        console.log('[ApprovalRequests] Fetching with department:', userData.department)
        var response = await fetch(`/api/marksheets?department=${userData.department}&status=dispatch_requested`)
      }
