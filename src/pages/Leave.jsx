...
function Leave() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true)
      const res = await fetch(`/api/leaves?studentId=${studentId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/leaves?action=create', {
        method: 'POST',
  ...
  const handleConfirmArrival = async (request) => {
    try {
      const response = await fetch(`/api/leaves?id=${request._id}&action=confirm-arrival`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      })
  ...
  const handleDeleteLeave = async (request) => {
    try {
      const response = await fetch(`/api/leaves?id=${request._id}&action=delete`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      })
