...
function Reports() {
  ...
  const generateDepartmentSummary = () => {
    ...
    marksheets.forEach(m => {
      ...
      // Grade distribution
      const grade = m.overallGrade || 'N/A'
      if (!overallGrades[grade]) {
        overallGrades[grade] = 0
      }
      overallGrades[grade]++
    })
  ...
  const generateClasswisePerformance = () => {
    ...
    marksheets.forEach(m => {
      ...
      const grade = m.overallGrade || 'N/A'
      if (!classwiseStats[classKey].gradeDistribution[grade]) {
        classwiseStats[classKey].gradeDistribution[grade] = 0
      }
      classwiseStats[classKey].gradeDistribution[grade]++
    })
  ...
  const generateSubjectAnalysis = () => {
    ...
    marksheets.forEach(m => {
      m.subjects?.forEach(subject => {
        ...
        subjectStats[subName].totalEnrollments++
        subjectStats[subName].totalMarks += marks
        subjectStats[subName].highest = Math.max(subjectStats[subName].highest, marks)
        subjectStats[subName].lowest = Math.min(subjectStats[subName].lowest, marks)
        
        if (marks >= 40) { // Assuming 40 is passing marks
          subjectStats[subName].passCount++
        }
      })
    })
    ...
    Object.values(subjectStats).forEach(stats => {
      stats.average = stats.totalEnrollments > 0 
        ? stats.totalMarks / stats.totalEnrollments
        : 0
      stats.passingRate = stats.totalEnrollments > 0 
            ...
            <div className="responsive-grid responsive-grid-2 lg:grid-cols-3">
              ...
              <div className="responsive-card group hover:shadow-xl transition-all duration-300 bg-white border-2 border-blue-100 hover:border-blue-300">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <HelpTooltip content="Shows overall statistics including status breakdown, year-wise distribution, and grade analysis for all marksheets." />
                </div>
                <p className="text-sm text-gray-600 mb-4 line-clamp-2">Complete overview with status breakdown, year-wise stats, and grade distribution</p>
                <button 
              ...
              <div className="responsive-card group hover:shadow-xl transition-all duration-300 bg-white border-2 border-green-100 hover:border-green-300">
                ...
                <p className="text-sm text-gray-600 mb-4 line-clamp-2">Detailed breakdown by year, section with dispatch rates and grade distribution</p>
                <button 
              ...
              <div className="responsive-card group hover:shadow-xl transition-all duration-300 bg-white border-2 border-purple-100 hover:border-purple-300">
                ...
                <p className="text-sm text-gray-600 mb-4 line-clamp-2">Performance metrics for each subject with average, pass rate, and grade stats</p>
                <button 
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                      ...
                      <div className="responsive-card bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <h4 className="text-base md:text-lg font-bold text-purple-900">Grade Distribution</h4>
                        </div>
                        <div className="space-y-3">
                          {reportData.data.overallGrades && Object.entries(reportData.data.overallGrades).map(([grade, count]) => (
                            <div key={grade} className="flex items-center justify-between p-3 bg-white rounded-lg hover:shadow-md transition-shadow">
                              <div className="flex items-center gap-2">
                                <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                                <span className="text-sm md:text-base font-medium text-gray-700">Grade {grade}</span>
                              </div>
                        ...
                        <div key={subject} className="responsive-card bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 hover:shadow-lg transition-all">
                          ...
                          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                            ...
                            <div className="bg-white p-3 rounded-lg">
                              ...
                              <p className="text-lg md:text-xl font-bold text-blue-600">{stats.passingRate.toFixed(1)}%</p>
                            </div>
                          </div>
                        </div>
                      ))}
                      ...
                      {reportData.data && Object.entries(reportData.data).map(([classKey, data]) => (
                        <div key={classKey} className="responsive-card bg-gradient-to-br from-green-50 to-green-100 border-green-200 hover:shadow-lg transition-all">
                          ...
                          {data.gradeDistribution && Object.keys(data.gradeDistribution).length > 0 && (
                            <div className="bg-white p-3 rounded-lg">
                              <p className="text-xs font-semibold text-gray-700 mb-2">Grade Distribution</p>
                              <div className="flex flex-wrap gap-2">
                                {Object.entries(data.gradeDistribution).map(([grade, count]) => (
                                  <span key={grade} className="text-xs font-medium bg-gradient-to-r from-green-100 to-green-200 text-green-800 px-3 py-1 rounded-full">
                                    {grade}: {count}
                                  </span>
                                ))}
                              </div>
