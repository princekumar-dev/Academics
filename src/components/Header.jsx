      ...
      if (role === 'hod') {
        ...
        if (hodId && department) {
          const [staffResponse, leaveResponse] = await Promise.all([
            fetch(`/api/staff-approval?action=pending&hodId=${hodId}`),
            fetch(`/api/leaves?department=${department}&type=leave`)
          ])
        ...
        const notificationResponse = await fetch(`/api/notifications?userEmail=${email}`)
        ...
        if (year && section && department) {
          try {
            const lateResponse = await fetch(`/api/leaves?department=${department}&type=late&status=requested`)
