    ...
    try {
      ...
      if (userRole === 'hod') {
        ...
        try {
          ...
          console.log('[NotificationRequests] Staff Response status:', staffResponse.status)
          console.log('[NotificationRequests] Leave Response status:', leaveResponse.status)
        ...
        const response = await fetch(`/api/notifications?userEmail=${auth?.email || auth.user?.email}`)
        ...
        if (auth?.year && auth?.section && auth?.department) {
          try {
            const lateResponse = await fetch(`/api/leaves?department=${auth.department}&type=late&status=requested`)
        ...
        if (!email) {
          ...
          const response = await fetch(`/api/notifications?userEmail=${encodeURIComponent(email)}`)
          const data = await response.json()
  ...
  const handleApprove = async (request) => {
    ...
    if (request.type === 'late_arrival') {
      ...
      try {
        ...
        const response = await fetch(`/api/leaves?id=${request.data.requestId}&action=acknowledge`, {
          method: 'PATCH',
    ...
    try {
      ...
      if (request.type === 'staff_account_approval') {
        response = await fetch('/api/staff-approval', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        response = await fetch(`/api/leaves?id=${request.data.requestId}&action=approve`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            hodId
  ...
  const handleReject = async (request) => {
    ...
    try {
      ...
      if (request.type === 'staff_account_approval') {
        ...
        response = await fetch(`/api/leaves?id=${request.data.requestId}&action=reject`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            hodId,
