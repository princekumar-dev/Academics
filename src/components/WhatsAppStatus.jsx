...
export default function WhatsAppStatus() {
  ...
  const fetchStatus = async () => {
    try {
      const response = await fetch(`${origin}/api/whatsapp-dispatch?action=status`);
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        console.warn('WhatsApp status fetch failed:', data.error || response.status);
        return;
      }
      ...
      console.warn('Failed to fetch WhatsApp status:', error.message);
    }
  };

  const fetchConnectionStatus = async () => {
    try {
      const response = await fetch(`${origin}/api/whatsapp-dispatch?action=connection-status`);
      if (!response.ok) {
  ...
  const fetchQRCode = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${origin}/api/whatsapp-dispatch?action=qrcode`);
      const data = await response.json();
  ...
  const performLogout = async () => {
    ...
    try {
      ...
      const response = await fetch(`${origin}/api/whatsapp-dispatch?action=logout`, {
        method: 'POST'
      });
  ...
  const performDeleteInstance = async () => {
    ...
    try {
      ...
      const response = await fetch(`${origin}/api/whatsapp-dispatch?action=delete-instance`, {
        method: 'DELETE'
      });
      
      console.log('Delete instance HTTP response:', {
      ...
      // Consider 404 and success both as successful deletion
      if (data.success || response.status === 404) {
        ...
        showError('Failed to delete instance', data.error || 'Unknown error');
      }
    } catch (error) {
